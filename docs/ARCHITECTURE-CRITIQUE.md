...
- **Reinforce engine-fat / theme-thin** and **stop calling it "Hugo-like."** Keep *all* security + UI logic in the live engine; the copied theme is presentation/registry/CSS only. This is the only answer to "scaffold-copy can't propagate security fixes" that actually holds. cairn already does most of this.
...
- **Govern the `CairnExtension` contract from day one** (narrow surface, versioned, extensions peer-dep only cairn-cms, build-time validation). Accept that **SvelteKit cannot provide package routes**; extensions contribute data + components, the site owns thin route shims (ideally generated).
- **Non-developer safety net:** server-side frontmatter validation *before* commit, a deploy-status signal, and a revert affordance. Today a bad save breaks the live build with no editor recovery.
...
and retires the hand-rolled JWT/token lifecycle: timing-safe compare, single-use, and TTL are all handled by a
maintained lib (see H3, C1). **Recommendation: the Architecture-Refinement pass should seriously evaluate replacing the
hand-rolled magic-link/session with `better-auth` (D1 adapter + magic-link plugin)** rather than hand-hardening
KV. The rest of cairn (GitHub-App commit, SvelteKit embedding, editor, registry) stays bespoke and unfilled.
...
comparison is **Carta vs bare CodeMirror 6**; **Carta is itself a thin Svelte wrapper around CodeMirror 6**.
...
component depends on the interface, not Carta's API. A later swap to bare CM6 then becomes a one-file change. **Reassess
...
KV is for "read at high rates, not immediately consistent," which is the opposite of what an auth nonce requires.
**Fix:** nonce in a Durable Object (atomic `verify()` delete-and-return), or D1 `DELETE … WHERE token=? AND expires_at>? RETURNING *`. A strong alternative: **adopt `better-auth` (D1 adapter + magic-link plugin)**, which handles this on strongly-consistent D1 and retires the hand-rolled token lifecycle entirely (see P2).
...
inbound mail. A GET-consumed magic link is **redeemed by the scanner before the user clicks**, leaving the user
with "invalid/expired": a self-inflicted auth DoS plus a confused security state. Documented against
...
**Fix (scoped):** catch 409, re-fetch, and if the file changed under them, show "this file changed since you
...
*already* mounts Carta client-only, which is the correct mitigation. This is a **guard, not a change**: enforce
...
*deprecated* partly over this fragmentation. **The only mitigation that holds is to keep all security-critical and
...
Scaffold-copy isn't a stylistic choice; **it's forced by the build model**. Users cannot "override theme
...
The contents API lists **≤1,000 entries per directory with no pagination**. Silent truncation means newer posts
vanish from the admin. Files above **1 MB** return `content: null` and require the Blobs API. A 2-posts/week
blog hits 1,000 in ~10 years, but a flat `posts/` dir is the default.
...
Save → commit → CI → deploy is 1–4 min with **no status signal**. A malformed save (bad frontmatter)
**breaks the live build with zero editor recovery**, and a developer must intervene. **Fix:** server-side
...
Workers `SubtleCrypto` accepts only PKCS#8; GitHub issues PKCS#1. cairn converts in-process. That's correct,
but string/PEM munging is brittle and **untested in-Worker**, and a key rotation could skip the conversion and
silently kill all commits. **Fix:** prefer `jose`/`workers-jwt` `importPKCS8`; add a `/admin/healthz` that signs a
...
500s. This is exactly the workspace-symlink case. **Already retired** (single root kit; live-verified Pass F2; kit's
...
survives scrutiny. Sequence the four CHANGE items into the build, and fold the design refinements into
the relevant passes:
