...
needs PostgreSQL + a Node server, which fails the embed-on-Workers criterion). **Build is justified.**
...
| **Site template** | The scaffold you create a new site from: a public SvelteKit/Tailwind/DaisyUI design + a component registry + a default `cairn.config.ts` adapter + sample content | **Scaffold-copy (starter model).** Copied into a new repo; the site owns + diverges; template updates do **not** propagate. *(Planned: `create-cairn-site`.)* **Not a WP/Hugo "theme"** (H2: Hugo overlays at runtime; SvelteKit/Vite resolve at compile time; closest live analogue is Astro Starlight; "theme" retired for the site-design concept). |
...
- **Cloudflare-first** for everything else: any CF primitive that fits is fair game, governed by "keep the core lean," not CF-avoidance.
  - **D1** *(auth: planned, auth-migration pass; non-content stores: sanctioned)*: **becomes the auth store** via better-auth (users/accounts, sessions, magic-link verification tokens; strongly consistent, §4). Also the sanctioned home for a future collection-definition store (R8), a rebuildable content/asset *index*, and ephemeral state. Never content.
  ...
  - **R2 / Images** *(future)*: media uploads (R7); default to **R2** (zero egress), URL-only in markdown (never binaries in git, M7). Browsing *existing* committed assets (R9) needs no new store, since git is the archive.
- **Reads:** the GitHub **contents API** today; **switching listing to the Git Trees API** (H4), because the contents API silently truncates directory listings at **1,000 entries** and returns `null` for files **>1 MB**. Reads are authenticated via the App install token (5000/hr; anonymous shared-IP reads 403 in prod, hotfix 0.3.1). **Collection sharding** (year/month) is deferred to a documented trigger (~500 files/collection).
