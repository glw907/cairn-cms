...
- **`commitFile` is single-file.** One contents-API PUT per save. Anything needing *atomic multi-file* writes (bidirectional relations, some i18n moves) would need the git-data **tree/commit** API — a real addition, not a tweak.

## Verdicts — is the door open?

Surveyed git-based peers (Sveltia, Decap, Keystatic, Tina) and file-based/traditional peers
(Statamic — the closest architectural cousin — WordPress, Ghost). Per capability: **door open?**,
...
discriminant. Don't build now — `draft` covers the requirement.
...
stores **a slug/ID string** in frontmatter and resolves at render time — no FK integrity. cairn can do
this today with a `text` field; a typed `relation` field (target `collection` + stored slug) would be a
clean, **additive** `CairnField` variant. The hard part is **bidirectional sync** (write the inverse
link into the other file) — that needs the multi-file atomic commit cairn doesn't have (`commitFile`
...
untouched. What's *hard* is Statamic-style **field-level inheritance** (show the English value greyed-out
in the French editor) — that needs the edit UI to fetch and merge two files. **Verdict:** door open for
...
*are your repo* — no runtime theme layer). Statamic's Starter Kit model **is** the Hugo model, and **is**
...
1. **Don't let `commitFile` ossify as single-file-only in consumers' mental model.** Bidirectional
   ...
   It's additive when needed — just don't design features assuming one-PUT-per-save is the only shape.
...
3. **Per-collection role scoping is the known next step for roles** — shape is `allowedRoles?` on the
...
4. **Revision/rollback is the cheapest, highest-value additive feature** and needs no contract change —
   the natural next capability if author-facing polish is wanted.
...
**Base to start from: [`scosman/CMSaasStarter`](https://github.com/scosman/CMSaasStarter)** —
Svelte 5 runes (`$props`, `{@render children?.()}`), MIT, pure DaisyUI semantic classes (theme-driven,
no hard-coded colors), `(admin)/(menu)/+layout.svelte` is exactly a drawer+navbar admin shell scoped to
...
> exist** (verified — `gh api` 404, no matching repo in search). Replace that reference with
> CMSaasStarter. **Lucia** (`pilcrowOnPaper/lucia`) is an auth *library* with no UI — nothing to mine
