...
**`auth.csrf-token-invalid`, a blocker.** An admin form submission carried no valid CSRF token, or
one that didn't match. This is almost always a stale browser tab or a browser blocking cookies for
the site, not a configuration problem.
...
**`auth.csrf-origin-mismatch`, a blocker.** A form submission outside the admin carried an
`Origin` header that didn't match your site, so cairn's own origin check refused it.
...
**A gap worth knowing about:** a site scaffolded with a current SvelteKit toolchain carries no
`svelte.config.js` file at all; that wiring now lives inside `vite.config.ts` instead. On
a site shaped that way, this check finds no file to read and reports a skip, not a pass, and a
skip looks the same as clean in the doctor's own summary line. A skip here means the check simply
didn't run, not that your CSRF hand-off is confirmed working. See
[Build a site by hand](../extend/build-a-site-by-hand.md) for where that wiring actually lives on
a current scaffold, and confirm it by reading your own `vite.config.ts` and
`src/hooks.server.ts` rather than by this check alone.

**Ask a developer:** this one needs a code change in two files your site's developer owns. Send
them [Build a site by hand](../extend/build-a-site-by-hand.md), which names both edit points.
...
sign-in tables, or holds no owner row at all, so no sign-in link can be minted for anyone.

**Act:** re-run `npx create-cairn-site --dir <your-site-directory>`. It creates the database,
applies every migration in order, and writes your owner row. If your site already went through
[Create your site](./create-your-site.md), this check is just confirming that stuck, not asking
you to redo it. If it still fails afterward, that's a developer's job; send them
...
your Worker's secret store and need a developer with a terminal and `wrangler` to read or change.
If your key was recently rotated, see
