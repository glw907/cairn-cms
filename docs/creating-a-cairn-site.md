...
> — it carried WordPress/Hugo "installable/swappable package" baggage that never matched this model.
...
| **Site template** | A starting point: public SvelteKit/Tailwind/DaisyUI design + default `cairn.config.ts` adapter + a component registry + icon set + sample content | **Scaffold-time copy** — `create-cairn-site` copies it into a new repo; *not* a runtime dependency |
...
overlay to emulate (SvelteKit #8896), so users **cannot** "override template files" the way Hugo
