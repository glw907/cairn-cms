...
Cairn keeps content as markdown in git, which is a poor fit for an image or a PDF, so media takes
a different path: the bytes live in Cloudflare R2, and the content commits only a small logical
reference that resolves to a real URL when a page renders. The exact reference grammar and
resolver API are in the [media reference](../reference/media.md); the admin's day-to-day library
workflow is in [Manage the media library](../guides/manage-the-media-library.md).
...
gets slower as a result. It's a well-known failure mode for a git-based CMS that stores its
binaries in the repo. Cairn avoids it by never putting the bytes there at all.
...
Content commits a logical handle, not a real media URL. The handle is keyed to a hash of the
file's own bytes, and a render step resolves it to a delivery URL at the moment a page is built or
served:
