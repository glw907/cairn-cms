...
That trade has an academic pedigree. Document-preparation research from the late 1970s onward (Scribe, TeX, and the SGML tradition) established that separating logical structure from visual presentation yields a consistency that direct formatting cannot, because appearance is decided once, centrally, rather than at every paragraph. Archival practice points the same direction: the Library of Congress's recommended-formats guidance prefers open plain-text encodings over proprietary binaries for anything meant to last. And computational academia, which needs documents that can be reviewed, diffed, and reproduced, standardized on plain-text authoring for the same reasons. Markdown is the lightweight end of that tradition.
