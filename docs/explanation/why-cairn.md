...
The trade has an academic pedigree. Document engineering has argued since the Scribe and SGML work of the late seventies that separating structure from presentation makes documents more consistent and more reusable, and version control adds a practical edge, since plain text is the one authoring format for which "what changed" has an exact, computable answer. Markdown is the lightweight end of that tradition.
