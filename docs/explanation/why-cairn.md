...
Mostly because a website is not a document. Plenty of great prose gets written in Word, and for a standalone document, where the author owns the presentation, WYSIWYG is the right tool. On a site, presentation belongs to the design system, not the author. It's a long-running argument that I'm not trying to win here. Cairn is an expression of which side of the argument I believe to be the better one.

WYSIWYG content on a site drifts: pasted formatting rides along, and authors make presentational choices ("make this big and blue") where the design system expects structure ("this is a heading"), so reconciling the two becomes developer work. Markdown makes the opposite trade. The author marks what something is, the site decides how it looks, and the syntax is a smaller hurdle than it appears (the marks look like what they mean, the toolbar types them for you, and cairn includes a friendly cheat-sheet for the rest). A little investment in markdown frees writers up to focus on writing and not layout.

The trade has an academic pedigree. Document engineering has argued since the late seventies that separating a text's logical structure from its presentation makes documents more consistent and more reusable: [Brian Reid's Scribe](https://en.wikipedia.org/wiki/Scribe_(markup_language)) built the first clean separation of document structure from a format database, [Knuth's TeX](https://en.wikipedia.org/wiki/TeX) let authors encode intent rather than ink positions, and the [SGML](https://en.wikipedia.org/wiki/Standard_Generalized_Markup_Language) tradition standardized descriptive markup. The clearest formal statement is Coombs, Renear, and DeRose's 1987 *Communications of the ACM* paper, which argued that descriptive markup frees authors to "focus their attention on the content," while presentational formatting works against them. [Markdown](https://daringfireball.net/projects/markdown/) (Gruber and Swartz, 2004) is the lightweight end of that tradition.

The modern evidence runs the same way. Computational academia converged on plain-text authoring because version control operates on lines of text, so plain text is the one format for which "what changed" has an exact, computable answer; the case is made with peer-reviewed backing in [Wilson et al.'s "Good Enough Practices in Scientific Computing"](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005510) and, for practitioners, in [Kieran Healy's plain-text guide](https://plain-text.co/). Archival practice points the same direction by degree: the criteria preservation institutions actually use (simplicity, self-documentation, freedom from rendering dependencies) favor plain text over even standardized rich formats.
