...
Some oddities are ordinary. A page you just published can take a short while to appear, because the live site rebuilds itself after each publish. A photo that looks fine in the preview but wrong on the page usually means the site's design handles that image size differently in context, which is worth showing the administrator rather than fighting alone. And if the spellchecker keeps flagging a name or a term your site uses constantly, the word can be added to the site's dictionary, where it stays accepted for every writer from then on. If the Tidy control you usually see has quietly disappeared, that's the editor being honest rather than broken: something is wrong on the administrator's side of the connection to Anthropic, and the control comes back on its own once that's fixed, rather than sitting there failing every time you try it.
