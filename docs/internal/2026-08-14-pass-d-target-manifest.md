...
21 `docsAnchor` string values in `src/lib/diagnostics/conditions.ts` carries the filename
prefix `cloudflare-readiness.md#`; all 21 need it changed to `is-it-working.md#` (the page
renames, it does not just move). The 16 distinct heading slugs after the `#` do not change;
they are enumerated in [§4](#4-the-live-ui-stance-the-carried-facts-and-the-fixes) and every
one must survive as a real heading in the new page.
...
| --- | --- | --- | --- |
| `is-it-working.md` keeps "its 20 anchors" | outline, `2026-08-14-docs-track-outlines.md` line 127 | **Wrong**, already corrected by the conductor before this manifest; re-confirmed here. | `src/lib/diagnostics/conditions.ts` carries 21 `docsAnchor` entries resolving to 16 distinct anchors, all `cloudflare-readiness.md#<slug>`; the 21st, `skill.admin-screens-stale`, carries no `docsAnchor` at all (it is `ALLOWLIST`-excused in `check-readiness.mjs`). |
