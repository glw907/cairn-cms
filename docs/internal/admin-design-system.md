...
- **Office list (the concept list view):** `ConceptList` is built on the admin toolkit: `PageHeader`
  ...
  - The page follows the F3 proximity-grouping scale (the named 4/8px steps documented as a comment
    ...
    composing `PageHeader` beside `AdminTable` inside the floating card recipe (`card-shell
    card-shadow`) follows the same F3 header rhythm: `PageHeader` sets the offset (`mb-10`,
    `gap-0.5`, the `meta` line at `type-meta`), and the card hugs that offset directly with no
    toolbar tier between them, `AdminTable`'s own wrapper carrying the horizontal scroll.
