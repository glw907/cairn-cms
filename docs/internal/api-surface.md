...
- `AdminShellData`: { public: true; siteName: string } | { public: false; siteName: string; user: { displayName: string; email: string; role: "owner" | "editor" }; concepts: NavConcept[]; customNav: ResolvedNavItem[]; pathname: string; canManageEditors: boolean; navLabel: string | null; theme: "cairn-admin" | "cairn-admin-dark"; collapsedNav: string[]; csrf: string; pendingEntries: Promise<{ concept: string; id: string }[] | null> }
...
- `BackendEnv`: { GITHUB_APP_PRIVATE_KEY_B64?: string }
- `CairnAdminDeps`: { auth?: { branding?: AuthBranding; send?: SendMagicLink; bootstrapOwner?: { email: string; displayName: string } }; tidy?: { client?: ((opts: { apiKey: string }) => TidyClient); timeoutMs?: number }; navFilter?: ((items: ResolvedNavItem[], ctx: { editor: Editor; event: ContentEvent }) => ResolvedNavItem[] | Promise<ResolvedNavItem[]>) }
