...
## Phase 6 sign-off: media part 2 retired (admin budget 125 -> 0, the terminal floor)

Phase 6 (media part 2) is complete (2026-06-30, plan
`docs/superpowers/plans/2026-06-30-admin-re-expression-6-media-part-2.md`). It swept the last 125
muted/subtle bracket references across the five media files and ratcheted the admin `retiredTokenBudget`
to **0**, the terminal floor. The admin component tree now holds zero `var(--color-muted)` /
`var(--color-subtle)` bracket references.

Four clusters, each measured and ratcheted exactly: `CairnMediaLibrary`'s slide-over + safe-delete
(125 -> 103), its replace + push-alt dialogs (103 -> 63, including the `:2322` site below), its
bulk-delete + orphan dialogs (63 -> 20, finishing the file), and the four small media components
(`MediaHeroField`/`MediaFigureControl`/`MediaCaptureCard`/`MediaInsertPopover`, 20 -> 0). The six dialog
and region a11y models were color-only edits, preserved verbatim: the non-modal slide-over
`role="region"` (no focus trap), the safe-delete/replace/bulk-delete `alertdialog`s, the deliberate
push-alt `role="dialog"`, and the orphan list as plain labelled native checkboxes. No native-primitive
adoption, no `@layer` fold (`cairn-admin.css` untouched, `componentsLayerCap` stays 14), no coupled-test
migration. The slide-over detail panel gained its own visual baseline
(`admin-media-detail-{light,dark}`); the existing 14 admin-visual snapshots regenerated byte-identical
(the zero-pixel proof).

**The `:2322` strikethrough (resolved: option b, the currentColor decoration idiom).** The push-alt
"alt was" diff row struck through old alt text with `decoration-[var(--color-muted)]/55`, a
text-decoration-COLOR at 55% opacity. It cannot fold to `text-muted` (that sets `color`, a different
property), and the named role layer has no decoration-color utility. It was rewritten to
`decoration-[color-mix(in_oklab,currentColor_55%,transparent)]`, the sanctioned currentColor decoration
idiom already used at `TidyReview.svelte:363` and `EditPage.svelte:1841/1946` (the `/55` is baked into
the mix; the gate does not count a `color-mix` over `currentColor`). No new `@utility`, no frozen-layer
change, no test touch. The recorded drift: the span carries `text-base-content`, so the strikethrough
tint shifts from muted-at-55% to base-content-at-55% on that one row, a near-identical, intended change.
The daisyui-a11y review confirmed it decorative under WCAG 1.4.1 (the `text-primary` "after" value and
the arrow icon carry the before/after meaning, not the line tone). The pixel-exact alternative, a named
`@utility decoration-muted-faint`, was declined: adding a role utility for one site cuts against the
final phase's whole point, removing bracket customization.

**The terminal floor and the residual.** With the budget at 0, the gate now blocks any new muted/subtle
bracket token anywhere in the admin tree. The retired-token migration is complete. The residual custom
surface is exactly the Tier-2 sanctioned set this ledger documents: the accessibility inks, the
theme-adaptive elevation pair, the editor system (the walled CodeMirror theme), the embed-anywhere
infrastructure, the two unlayered forced workarounds, the `.btn-primary` lift, and the
`--color-muted` / `--color-subtle` guaranteed-value vars behind the frozen `text-muted` / `text-subtle`
