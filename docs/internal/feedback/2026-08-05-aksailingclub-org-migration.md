...
[the RC blocker](./2026-08-05-rc1-worker-condition-defect.md). The migration branch was complete
and green on `check`, `test`, `build`, and `cairn-audit`, and was held at this defect until
`0.94.0-rc.2` cleared it on 2026-08-06 (the amendment under the seam-fit table).
...
| `/cloudflare` | **Yes, as of `0.94.0-rc.2`.** Under `rc.1` this was **No**, for the same reason as `/auth-crypto` below: it did not start on Workers as published. `rc.2`'s `worker` condition clears it, and the clearance is proven against the published artifact rather than a patched copy (see the amendment below the table). Both primitives are strictly stricter than the copies they replaced. `verifyTurnstile` takes `(token, secret, opts)` where the site's copy took `(token, ip, secret)`; two adjacent string parameters swapping order is a silent hazard, and the only thing that caught it here was that the site changed every call site at once. | `RATE_LIMIT_MESSAGE`, the user-facing refusal copy, which is the site's and should be. The Turnstile site key and the `Window.turnstile` ambient type stay site-side too; the seam covers verification only. | None. No call site passes `hostname` or `action`, which is a gap on the site's side, not the seam's. |
| `/auth-crypto` | **Yes, as of `0.94.0-rc.2`.** Under `rc.1` this was **No**: the subpath did not work on Cloudflare Workers as published (see the RC blocker above), and the row could only describe the fit measured against a locally patched copy. `rc.2` fixes it and the measurement is now against the registry artifact (see the amendment below the table). Yes for the cryptography. The server-only `browser` condition is the whole first-gate-failure story above, which is a fit question only in the sense that adopting the subpath surfaces a coupling a site may not know it has. | A thin naming layer, `member-auth/lib/crypto.ts`, that keeps the member store's own cookie base names and its deliberately-different 15-minute token TTL. The cryptography underneath is gone; only the domain naming remains. `tokensMatch` had been reimplemented twice more, in `auth.ts` and `portal-action.ts`, and both are now the engine's. | None. |

**Amendment, 2026-08-06: the two subpath rows now read against `0.94.0-rc.2` from the registry.**
The site repinned to `rc.2` exactly, deleted `node_modules`, and reinstalled with `npm ci`, so
nothing of the diagnosis-time patch survived into the measurement; the installed exports read
`types > worker > browser > default` on both subpaths. `check` (0 errors, 0 warnings), 2057 tests,
and `build` stayed green, and the Playwright suite started a Worker and ran all 75 specs, where the RC
blocker had allowed none of them to reach a request. Every functional spec passed: the authenticated admin session, both
join and class-door paths, both member-portal session specs, all four waivers signing specs, and
the two `.ics` feeds. Those exercise both subpaths server-side under `workerd`, which is the proof
no gate short of a started Worker could give. The 60 remaining failures are all
