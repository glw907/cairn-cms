# cairn-cms STATUS archive (2026-05-28 rebuild start through the 0.78.2 cut, 2026-07-01)

Superseded "Immediate next action" entries moved out of the rolling `docs/STATUS.md` on 2026-07-01,
newest first, verbatim. The rolling doc keeps only the current entry; per-plan post-mortems live with
the plans under `docs/superpowers/plans/`.
...
`retiredTokenBudget` to **0**, the terminal floor. The admin component tree now holds zero
`var(--color-muted)` / `var(--color-subtle)` bracket references. The residual custom surface is only the
Tier-2 sanctioned set (the a11y inks, the elevation pair, the walled editor system, the embed-anywhere
infrastructure, the two unlayered workarounds, the `.btn-primary` lift, and the guaranteed-value
`--color-muted` / `--color-subtle` vars behind the frozen `text-muted` / `text-subtle` role utilities),
owned on purpose. Plan + post-mortem:
`docs/superpowers/plans/2026-06-30-admin-re-expression-6-media-part-2.md`; the ledger Phase 6 sign-off is
in `docs/internal/design/2026-06-29-custom-surface-ledger.md`.

**Phase 6 (four clusters + a baseline).** Executed through one background Workflow that ran the four
`cairn-implementer` clusters as a strict in-order ratchet (shared budget file; clusters 1-3 edit the same
`CairnMediaLibrary.svelte`). Budget **125 -> 103 -> 63 -> 20 -> 0**: the slide-over + safe-delete, the
replace + push-alt dialogs (incl. the `:2322` site), the bulk-delete + orphan dialogs (finishing the
file), then the four small media files. The six dialog/region a11y models were color-only edits, preserved
verbatim. The slide-over detail panel gained its own visual baseline
(`admin-media-detail-{light,dark}`); the existing 14 admin-visual snapshots regenerated byte-identical
(the zero-pixel proof). Gate green throughout (`check` 0/0, `npm test` 2867 exit 0,
`check:custom-surface` PASS both trees at 0, `check:comments` OK, showcase admin-visual e2e 16/16 fresh
`CI=1`). Review gate clean: code-simplifier no-op, svelte-reviewer and daisyui-a11y-reviewer both clean.

**The `:2322` decision: option (b), confirmed by Geoff.** The push-alt "alt was" strikethrough used
`decoration-[var(--color-muted)]/55`, the one form the named role layer cannot express. Rewritten to the
sanctioned currentColor idiom `decoration-[color-mix(in_oklab,currentColor_55%,transparent)]` (no new
utility, no frozen-layer change, no test touch). The recorded drift: the strikethrough tint shifts from
muted-at-55% to base-content-at-55% on that one diff row, decorative under WCAG 1.4.1, not separately
baselined (rendering the row needs a four-step interaction on seeded custom-alt media).

**Released as 0.78.1** (Geoff's explicit instruction, 2026-06-30), overriding the sweep's standing
hold-and-batch posture. This is the deliberate publish of the admin re-expression window (Phases 2-6, all
admin-internal CSS and components a consumer never imports). It sizes as a **patch**: no public surface
change, no behavior change, so an upgrading site needs no action. Phases 0-1 already shipped in `0.78.0`.

**NEXT = the starter-template track, then the docs phase.** The template track re-expresses the showcase
starter template in native DaisyUI 5.6 / Tailwind 4 (sequenced after the admin primitives publish into the
showcase `file:` dep, now done). The docs phase publishes the role vocabulary (the `text-muted` /
`text-subtle` utilities plus the recipes) as the versioned seam in `admin-design-system.md`. Spec:
`docs/superpowers/specs/2026-06-29-admin-idiomatic-re-expression-design.md`.

**The CodeMirror integration is a separate future pass (Geoff steer, 2026-06-30).** Better-integrate
CodeMirror so the editor feels like a fully integrated part of cairn and shares its design language, while
keeping CM easy to upgrade (narrow seams) and not subverting CM's battle-tested editing UX for design
purity. The design must START with a prior-art review of how other systems integrated CM while preserving
their identity and upgrade seams. Framing: `docs/internal/cm-editing-surface-alignment.md` + ROADMAP
...
`CairnAdminShell` (cairn's chrome moved out of the internal `AdminLayout` into a `/admin/+layout` the consumer
mounts; `CairnAdmin` renders bare inside it), behind the editor login, with a data-only `adminNav` sidebar
entry. `requireOwner` narrowed to a minimal event; `shellLoad`/`AdminShellData` (streamed pending count,
public early-return); `adminNav` validated against the `parseAdminPath` collision authority; the showcase
`Signups` screen proves it end-to-end (e2e green against the worktree engine). **Version is `0.77.0`, held
unpublished until Plan 2 lands** (the two plans ship as one minor). The full gate is green and the reviewer
fan-out (svelte, web-auth-security, daisyui-a11y) found no blocker; the web-auth review confirmed the
`requireOwner` narrowing is safe and a custom `/admin` POST is genuinely CSRF-protected in production.

**Canonical artifacts:** spec `docs/superpowers/specs/2026-06-28-cairn-developer-extensibility-redesign-design.md`;
**Plan 1** `docs/superpowers/plans/2026-06-28-cairn-extensibility-1-admin-shell-and-nav.md` (COMPLETE, post-mortem
appended); **Plan 2** `docs/superpowers/plans/2026-06-28-cairn-extensibility-2-enforced-boundary.md` (WRITTEN,
queued).

**NEXT = EXECUTE Plan 2 (enforcement)** task-by-task on a fresh worktree off `main`: the `check:surface` `.d.ts`
snapshot gate (committed per-subpath Markdown surface, regenerate-to-disclose); the `check:reference`
tier-marker assertion + the Extension-API/Scaffold-API labels across every reference page; the `cairn-doctor`
mount-shape check; the `LayoutData` removal (dead export, breaking, rides the same `0.77.0` `Consumers must`);
the breaking-in-0.x release signal. Dispatch each task to `cairn-implementer` (Sonnet); main loop reviews each
diff and clears the gate between dispatches; `npm run package` before `npm test` / `check:surface` in a
worktree. The snapshot format and tier rule are settled in the plan (no open decisions). **Cut the held
`0.77.0` release covering both plans only after Plan 2 lands**, with the from-scratch consumer build + e2e and
the **live admin smoke (owed from Plan 1)** done first. Use `cairn-pass` for pass-start/end. The DX steer
(`cairn-extensibility-dx-steer` memory): cairn must be an easy, non-restrictive starting point; DX is weighed
first-class WITH the charter premise check.

**Worktree note (Plan 1 lesson):** in a feature worktree the showcase `node_modules` symlinks to the main
checkout, so a showcase e2e resolves the engine to MAIN, not the worktree, until a from-scratch `npm install`
in the worktree showcase repoints the `file:` deps. Run that install before the showcase e2e in a worktree.
...
`hydrateIslands` runtime on a new `./islands` subpath; the showcase converter flagship. Additive: a site
...
`TidyReview`); the `ConceptList` error live-region re-announce (one polite region, zero-width nonce,
visible boxes drop `role="alert"`); a module-private engine `DEFAULT_ICON_BY_ROLE` fallback in
`registry.ts` (consulted by `defaultIcon` only for a def with an icon field) + the icon guidance; real
JSDoc on the two `rehype-dispatch` helpers; the admin-prose gate extended to the two `.ts` copy modules; a
new `check:dev-package` gate over `packages/**` in CI (which found + fixed 21 TSDoc errors in the
dev-package); the friction-log prune; and the 0.68.0 bump + lockfile self-version sync. A workflow chain
started it but the Task 2 browser-test agent hung on a reactivity bug; recovery finished in the main loop
(see the post-mortem's execution note). Both reviewers ran: the a11y reviewer caught a real cap-cascade
blocker (fixed). Plan + post-mortem:
`docs/superpowers/plans/2026-06-26-cairn-engine-hardening-engine-misc.md`.

**Verified (Plan 2).** `npm test` exit 0 (2533); `npm run check` 0/0; `check:comments`, `check:prose`,
`check:dev-package`, and the doc gates (`check:reference`, `check:reference:signatures`, `check:package`,
`check:docs`, `check:version` minor) all green, re-gated after the review fixes and a code-simplifier dedup.

**Hardening Plan 1 (validator/fields parity) DONE as `0.67.0`.** The two validators single-source their
constraint logic in the internal `src/lib/content/field-rules.ts`; v2 `fieldset` reached v1 text/date
constraint parity + declaration-time pattern compile + parsed-YAML symmetry + multiselect lone-scalar
coercion; a v1-vs-v2 parity matrix proves agreement. Plan + post-mortem:
`docs/superpowers/plans/2026-06-26-cairn-engine-hardening-validator-parity.md`.

**NEXT: the Contract v2 cutover, drafted and GATED ON GEOFF'S REVIEW before execution.** It is breaking
and high-blast-radius (Geoff: no hurry to cut over), so unlike the hardening passes it does not land
autonomously. The cutover wires `fields.*`/`fieldset` into the live editor, validator, delivery, and
manifest, migrates the showcase off `defineFields`, and removes `FrontmatterField`/`defineFields`/
`validate.ts` (freeing the v2 `*Field`/`Infer` barrel names the additive phase held back). Spec:
`docs/superpowers/specs/2026-06-25-cairn-contract-v2-design.md`. Plan series after the cutover:
references → object/array + adapter + component unification → backend → render + islands.
...
- **Hardening Plan 1 (the cutover's spine): validator/fields parity + unification.** DONE as `0.67.0`
  ...
  `docs/superpowers/plans/2026-06-26-cairn-engine-hardening-validator-parity.md`.
- **Hardening Plan 2: engine-misc.** DONE as `0.68.0` (see the immediate-next-action block above). Plan +
  post-mortem: `docs/superpowers/plans/2026-06-26-cairn-engine-hardening-engine-misc.md`.
...
**Version note:** Contract v2 Plan 1 took `0.66.0`; hardening Plan 1 took `0.67.0`; hardening Plan 2 took
`0.68.0`. The cutover takes the next version (likely a minor; it is breaking within the `0.x` window). The
deferred pre-B3 engine/DX slot re-numbers when it resumes after the contract stabilizes.
