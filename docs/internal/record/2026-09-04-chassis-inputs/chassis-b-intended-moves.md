...
- `error404` 320/390/768/1440/2560 light and dark: the band-to-footer seam closes. RULING 1's
  ...
  base-200 ground. `src/chassis/composition.css` adds `.cairn-band + .site-footer { margin-top:
  0; }`: the band owns its own exit, and only the direct-sibling case is touched, so a band
  followed by a differently-toned block keeps its footer's own separating margin. The
  styleguide's own Band demo sits nested inside the Composition section, never a direct sibling
  of `.site-footer`, so it is unaffected (confirmed below).
...
- `magick compare -metric AE` on `styleguide`'s full tile set (79 tiles across five widths and
