...
proposal, an API entry as an API entry, and an editor's welcome page as a welcome page. A
...
- Two entries per page type is the ceiling, and one is the floor: a page type with no
  entry has no draft.
