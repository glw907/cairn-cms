...
| `TestExportedSurfaceIsPinned` | `tool/internal/render/purity_test.go:258` | Pins `render`'s exported names (`:224-246`). retire-1 adds `MarshalDoctor`, its input type, and `DoctorSchemaVersion` to that list. Names no docs path, listed because pass A's task 3 and task 8 both edit this file's package. |
