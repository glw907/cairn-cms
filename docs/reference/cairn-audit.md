...
| `stock-default-hazards` | Five stock daisyUI patterns cairn's own recipes replace: `badge-ghost`, the focus-driven bare `.dropdown`, a native `disabled` on a guarded button, a flat `base-300` card border, and the `cairn-btn-guarded` class itself, retired **advisory** at this tier until `0.98.0` promotes it to error, in favor of wrapping the control in `Tooltip` for its reason text. Each finding names the refuted alternative and cites where the decision lives |
...
| `log-secret-field` **advisory** | The same call heuristic, over each call's second, fields argument: a property key that whole-matches (never a substring) a member of `REDACTED_LOG_KEYS` (`@glw907/cairn-cms/log`). The runtime already redacts that field's value, so this rule exists for the case redaction can't reach, the same secret's value also written directly into the message string |
...
sixteen questions came back clean. It isn't an accessibility result.
