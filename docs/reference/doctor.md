...
looks for `svelte.config.js`, since one check reads the CSRF handoff there. A current bare
`sv create` scaffold writes no `svelte.config.js` at all: the adapter, and any
`checkOrigin: false` setting, live inside `vite.config.ts`'s `sveltekit({ ... })` call instead, so
that one check finds nothing to read there (see the `config.csrf-disable` row below).
`create-cairn-site` bakes its template from this repo's own `examples/showcase`, which carries
`svelte.config.js`, so a `create-cairn-site` scaffold always has the file this check reads. In a repo
