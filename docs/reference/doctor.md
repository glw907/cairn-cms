...
Twenty checks run by default. Two opt-in flags add more: `--send-test` the live email send and
...
| `auth.store` | `auth.store-unreachable` | The `AUTH_DB` D1 database answers, the `editor`, `magic_token`, and `session` tables exist, `magic_token` carries the `nonce_hash` column `migrations/0004_login_nonce.sql` adds (an un-migrated database fails here, naming the migration, because the engine's sign-in binds every token to that column), and at least one owner-capability row is present (every declared role mapped to owner capability, `owner` when the site declares none). | No API token or account id, or the wrangler config carries no `AUTH_DB` `database_id`. |
