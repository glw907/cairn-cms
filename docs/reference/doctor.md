...
| `edge.hsts` | `edge.hsts-off` | The zone's own HSTS setting is enabled with a max-age of at least 30 days. This reads the zone, which covers every route. cairn's admin responses carry their own `Strict-Transport-Security` max-age whatever the zone says, so a failure here names the rest of the site rather than `/admin`. | No API token, or no from-address. |
