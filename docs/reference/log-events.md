...
| `turnstile.verify_failed` | warn | `verifyTurnstile` returns `false` for a reason worth diagnosing: a rejected fetch, a non-200 siteverify response, an unparseable body, or a `hostname`/`action` mismatch. An ordinary `success: false` (a genuine bot rejection) logs nothing, since that is the function working. | `reason` (`request_failed`, `bad_status`, `unparseable`, `hostname_mismatch`, or `action_mismatch`), `error` on `request_failed`, `status` on `bad_status`, `expected` and `actual` on a mismatch |
