...
| `auth.access.denied` | warn | The site's declared access map denies a request: a site's own route through `requireAccess` (no rule at all for the target, or `canReach` denies the session's role), one of the engine's own gated screens (`canReach` denies the session's role for its screen id), or a `createSectionAction`-wrapped action's own 403 branch (same map, same predicate). | `email`, `role`, `target` |
...
| `admin.action.rate_limit_absent` | warn | A `createSectionAction`-wrapped call configures a rate limit whose binding resolves to nothing, or whose `limit()` call throws: the check degrades to open (never blocks) rather than 500ing. | `path`, `action`, `entity`, `error` on a thrown `limit()` call |
...
| `admin.action.misconfigured` | error | A `createSectionAction`-wrapped call returns `fail(500)`: `config.resolveDb` returned `undefined`, or `event.locals.cairnAccess` was never attached (the guard never ran on this route). | `path`, `reason` (`db_not_bound` or `access_map_not_attached`) |
