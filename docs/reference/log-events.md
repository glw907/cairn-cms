...
| `admin.action.audit_sink_failed` | error | The site's `event.locals.auditSink` throws synchronously when `ctx.audit` invokes it. The wrapper catches the throw, so the action still completes and returns the handler's own result; only this record's trail past `admin.action.audited` is at risk. Omits `record.detail` (unlike `admin.audit.sink_failed`, which persists the whole truncated record): `detail` can hold arbitrary site data, and the untruncated original already logged as `admin.action.audited` before the sink ran. Distinct from `admin.audit.sink_failed`, which fires when the packaged `createD1AuditSink` itself fails to persist, a failure that sink already catches before it can reach this call site. | `path`, `action`, `entity`, `entityId`, `editor`, `error` |
