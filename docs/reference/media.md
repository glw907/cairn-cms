...
| `VariantSpec` | Extension API | `interface VariantSpec { width?; height?; quality?; fit?; gravity?; format?; upscale? }` | A single image variant: the resize and format directives Cloudflare Images applies to the original bytes. `fit` accepts `scale-down`, `contain`, `cover`, `crop`, `pad`, `aspect-crop`, `scale-up`, or `squeeze`; `upscale` (`interpolate` or `generate`) controls the enlargement algorithm for a fit mode that upscales. |
