...
Anything proposed here must be a plugin a site wires into its own `vite.config.ts`. This subpath
holds the build-time manifest plugin. The `cairnManifest()` plugin evaluates a site's
