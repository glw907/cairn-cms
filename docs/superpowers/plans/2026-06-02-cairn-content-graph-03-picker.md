...
Executed subagent-driven on `main`, one `cairn-implementer` per task (Sonnet), commits `9614b0a..d6aad7e` (the ten plan tasks), plus a simplifier commit `0c43fb0` and a test-hardening commit `6485e37`. Local only, not pushed, not published. Version bumped to `0.19.0` (additive). The pass delivers the editor link picker end to end: an author inserts a `cairn:` internal link two ways, a "Link to page" dialog and a `[[` autocomplete, both reading the `linkTargets` Plan 2 ships to the editor and both writing `[Display](cairn:<concept>/<id>)`.

**What was built.** `formatCairnToken(ref)` in `links.ts` is the inverse of `parseCairnToken`, so the picker and the autocomplete write exactly the token the resolver reads back. `insertInlineLink(doc, from, to, href, title)` in `markdown-format.ts` is a pure inline transform that wraps a selection as the display text or inserts the title when nothing is selected, with no block padding. `link-completion.ts` holds the pure `matchCairnTrigger` (the `[[query` matcher, stopping at a closing bracket or newline) and `linkCompletions` (a case-insensitive title substring filter, grouped by concept, drafts marked, the apply text the full link), plus `cairnLinkCompletionSource(targets)`, a thin CodeMirror `CompletionSource` adapter. `MarkdownEditor` gained two seams: `registerInsertLink` (an inline, selection-aware insert symmetric with `registerInsert`) and a generic `completionSources` prop wired through `autocompletion({ override, interactionDelay: 0 })`. `LinkPicker.svelte` is the "Link to page" dialog, mirroring `ComponentInsertDialog`'s native-`<dialog>` a11y conventions. `EditPage` registers the completion source and the inline insert, and renders the picker beside the component dialog. `formatCairnToken` and `LinkPicker` are exported from the package.

**Gate at the tip (`6485e37`):** `npm run check` 771 files 0/0, `npm test` 105 files / 537 tests exit 0 (green across three consecutive full-suite runs after the flake fix below), `check:package` all-green across all five entries with no export-condition change.

**Execution deviations locked in.** Task 1's `@codemirror/autocomplete` installed at `6.20.2` (pinned `^6.20.2`); the relock handled the workspace-hoist gotcha (move the workspace-root manifests aside, `npm install --package-lock-only` in cairn-cms, restore them) and synced the stale lock `version`. Task 6's `CompletionSource` type is referenced inline as `import('@codemirror/autocomplete').CompletionSource[]` rather than a top-of-script `import type`, because the `editor-boundary` test's `STATIC_EDITOR` regex flags any `import ...` line including the fully-erased `import type`; the inline form matches the file's existing `import('@codemirror/view').EditorView` idiom. Task 6 also set `interactionDelay: 0` on the autocompletion so an immediate Enter accepts the option rather than being swallowed by the default accidental-accept guard, and the test types `[[[[Ab` because `userEvent.keyboard` escapes a literal `[` as `[[`. Task 8 added `linkTargets: []` to a sibling `EditPage-insert.test.ts` data object: mounting `LinkPicker` makes `EditPage` read `data.linkTargets` eagerly (the `groups` derived runs on mount), which surfaced a latent contract gap where that test omitted the required key; the fix is in-contract, not a guard around an undefined the type forbids.

**Flake found and fixed at the gate.** The Task 6 autocomplete end-to-end test accepted the completion with an Enter keystroke. Under full parallel browser load CodeMirror's accept handler is sometimes not yet bound when the keystroke lands, so Enter fell through to a newline and the test failed about half the time (green in isolation, green on a re-run, red under load). Waiting for the selected option before Enter narrowed the window but did not close it. The fix (`6485e37`) accepts by clicking the option, which drives CodeMirror's mousedown-apply deterministically and proves the same completion seam without the keystroke race. The Enter-accept contract is CodeMirror's own built-in, not this pass's code, so clicking is a faithful end-to-end proof. Three consecutive full-suite runs are green after the change.

**Review gate.** The simplifier made one cosmetic consistency fix (the `{#each}` key as a template literal, `0c43fb0`) and reasoned against extracting the concept-section logic shared between `LinkPicker` and `link-completion.ts`, since the two live in different layers and emit different shapes; the parallel reads as incidental. `svelte-reviewer` (Opus) returned no Critical or Important: the `$state.raw` seam, the `$derived` completion source, the `groups` derived, and the CodeMirror seams are all correct. `daisyui-a11y-reviewer` (Opus) returned ship-it: DaisyUI 5 / Tailwind 4 clean, and the dialog plus the autocomplete popup match or extend the established `ComponentInsertDialog` a11y baseline (native `<dialog>` focus trap and Escape, `aria-haspopup`, `aria-labelledby`, the searchbox label, the draft conveyed as text not color, and CodeMirror's built-in combobox ARIA on the popup). A high-effort seven-angle `/code-review` surfaced no Critical or Important; its two convergent findings are carried below.

**Live admin smoke: carried fast-follow.** The showcase runs `adapter-node`, not a Worker, so there is no `wrangler dev` admin Worker to smoke. The browser component tests cover the dialog and the autocomplete here, and the interactive smoke (open the dialog, pick a target, type `[[` and accept, confirm the inserted link in a real browser) is best run during the ecnordic migration against that site's real Worker.

**Carried follow-ups for Plan 4 (the lifecycle pass).**

- **Unescaped brackets in link display text.** `linkCompletions`'s `apply` and `insertInlineLink`'s empty-selection branch wrap an author's title in `[...]` without escaping. CommonMark allows balanced brackets in link text, so a title like "Q3 [2026] Recap" or "FAQ (general)" renders fine; only an unbalanced `[` or `]` in a title breaks the link, and it self-corrects in the preview. A correct fix escapes the title-derived display text but leaves a user's live selection as-is, so it wants its own test-first task (balanced, unbalanced, and selection cases). Narrow under the trusted-editor model, below Important.
- **`insertLink` no-op before the editor mounts.** When CodeMirror has not finished its dynamic import, the textarea fallback is showing and `insertLink` returns without inserting, so picking a link in that sub-second window closes the dialog with no effect. This matches `applyFormat`'s pre-mount behavior (the toolbar's format buttons), the closest analog; only the block-insert `insertAtCursor` has a raw-value fallback, which a selection-aware transform cannot share. A fallback that appends `[title](href)` to the raw value is possible if the window proves reachable in practice.
- **`matchCairnTrigger` has no syntax-tree awareness.** The `[[` trigger fires on any open `[[` on the line, including inside a fenced code block, where `extractCairnLinks` (mdast-based) would not match. Typing `[[` in code shows the popup; Escape dismisses it. Acceptable as an insert gesture, recorded for completeness.
- **Section-order tiebreak.** `LinkPicker`'s group sort breaks a tie between two unlisted concepts by `localeCompare` on the raw concept id, which can disagree with the displayed heading for a site with concepts beyond pages and posts. Cosmetic, reachable only past the two built-in concepts.
