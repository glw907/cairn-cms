...
## Post-mortem (LANDED 2026-06-07, `0.33.0` unpublished)

The plan ran subagent-driven on `main` directly, one `cairn-implementer` per task on Sonnet (the tasks were mechanical and well-specified, so no Opus override). All seven tasks landed as written. Ten commits `c0280e9..b87cfd3`: seven task commits, one pre-existing-defect fix, one simplifier pass, one review fold-in.

**What was built.** A dev-only `chrome-guard.ts` exports `detectChromeWrap(root)`, a pure DOM walk that returns a diagnostic when a width-constraining ancestor sits between the admin root and `<body>`, and `warnIfChromeWrapped(root)`, a wrapper gated behind `import.meta.env.DEV` that logs one `console.error`. Both admin roots (`AdminLayout`, `LoginPage`) call it on mount against a `bind:this`-bound `data-theme` wrapper. A unit test pins that `cairn-admin.css` is imported only by the admin roots, so its document-global `@keyframes` and `@property` rules load only on `/admin`. The showcase gained a `(site)` route group with plain-CSS chrome, which proves the admin renders fully styled on a site that uses neither Tailwind nor DaisyUI. The route pattern is documented in `docs/admin-route-structure.md` and taught in the tutorial. The minor bumps `0.33.0` with a `Consumers must:` line.

**Two reconciliations the execution surfaced.**

1. *Three admin roots, not two.* Task 3 found that `ConfirmPage.svelte` also imports `cairn-admin.css`, so the boundary covers three roots. The implementer widened the assertion to the real set rather than weakening it, per the task's own instruction, and the Task 7 changelog was reconciled to say "the admin roots" with no count. The guard wiring (Task 2) stays scoped to `AdminLayout` and `LoginPage` per the plan; the confirm page is a brief interstitial, not a full shell, so the chrome-wrap risk there is low and out of the plan's stated scope.

2. *A pre-existing gate defect.* `main` carried two `svelte-check` errors in `AdminLayout.test.ts` (`document.querySelector('dialog.modal')?.open` types as `Element`), introduced by the design-identity arc fold-in `a76aa8b` and not caught because the arc's reported gate did not re-run `check` at the final tip. They blocked the plan's 0/0 exit criteria and sat in the very file Task 2 modifies, so they were fixed first as a standalone commit (`89abb78`, a typed `querySelector<HTMLDialogElement>`). STATUS's "0/0 at a76aa8b" was inaccurate.

**Verification evidence.** Gate green at the tip `b87cfd3`, run first-hand: `npm run check` 823 files 0/0, `npm test` 121 files / 758 tests exit 0, `check:reference`/`check:package`/`check:docs` all exit 0. The showcase builds clean (`npm run package` then the showcase `npm run build`, both exit 0) and the preview smoke returns `home 200`, `admin 200`, with the `<nav aria-label="Site">` chrome on the home page and the admin rendering outside it.

**Review gate.** The simplifier removed a redundant second `getComputedStyle` call (`373f24a`). The `svelte-reviewer` found no Critical or Important issue and confirmed the `$state`/`bind:this`/`onMount`/`import.meta.env.DEV` pattern is SSR-safe and the `(site)` routing is sound. The `daisyui-a11y-reviewer` flagged one Important item: the showcase header links had no `<nav>` landmark (WCAG 1.3.1). Both reviewers' material findings folded in as `b87cfd3`: the nav landmark (flex moved from `.site-header` onto the nav), a skip of non-constraining `max-width` values (`100%`, `100vw`) so a host's defensive wrapper does not trip the dev guard, and a fixture-only note on the test's `data-admin-root` marker. The demo-appropriate a11y Minors (skip link, `aria-current`) were left as the reviewer advised.

**Live admin smoke: judged not proportionate.** The plan touches `/admin`, but the only runtime addition is a dev-only `console.error` that compiles out of production and changes no rendering. No auth, session, cookie, or production-render path changed. The showcase preview smoke already proved `/admin` serves 200 and renders outside the `(site)` chrome. A full `wrangler dev` D1 session smoke would verify behavior this pass did not alter, so it was skipped, the same call plan 1 made for a change with no component-markup, auth, or Worker surface.

**This closes the plan-1 global at-rule carry-forward.** The leak is contained by isolation, not name-mangling: the sheet is code-split to the admin roots, so it loads only on `/admin`, and the route pattern keeps host CSS off `/admin` from the other side. The boundary test pins the import side.

**Next.** Plan 3 was the last engine plan of the admin-stands-alone initiative. The next action is the publish decision for the held window (`0.30.0` through `0.33.0` over the `0.29.0` registry `latest`), then the two production-site retrofits (907.life, ecnordic.ski), each a separate `site-pass` after the engine version publishes.
