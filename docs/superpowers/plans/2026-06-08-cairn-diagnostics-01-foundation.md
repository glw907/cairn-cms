...
## Post-mortem (landed on `main` 2026-06-09)

Pass 1 landed on `main` directly, subagent-driven, one `cairn-implementer` per task. Tasks 1 through 6 each ran a fresh implementer test-first and cleared the full gate before the next dispatched; Task 7 (pure verification, no code change) ran inline. Seven task commits `35825cb..8cbeacb`: the registry, the `CairnError` primitive, the internal barrel, the shared admin-response extraction, the condition renderer, and the guard rewire. No simplifier or review-fold-in commit was needed (both came back clean).

**What was built.** An internal `src/lib/diagnostics/` module: `conditions.ts` (the `CairnCondition` registry with three guard conditions, `condition(id)`, `allConditions()`), `error.ts` (the `CairnError` throw primitive), and `index.ts` (the internal barrel, re-exported from no public subpath). On the SvelteKit side, `admin-response.ts` holds the extracted `applySecurityHeaders` and `brandedAdminPage`, and `condition-response.ts` holds `renderConditionResponse(id, ctx)` plus the `REASON_CONDITION` map. The guard's three rejection branches (`origin`, `https`, `csrf`) now route through the renderer, and the three local helpers plus the now-unused `httpsRequiredPage`/`csrfRequiredPage` imports were removed. The `guard.rejected` log reasons are unchanged (the public log contract).

**Verified (evidence).** Gate green at the tip `8cbeacb`, run first-hand: `npm run check` 853 files 0/0, `npm test` 135 files / 832 tests exit 0, and `check:reference`/`check:package`/`check:docs` exit 0 with no edit (the internal-surface decision held). The pre-existing `auth-guard.test.ts` is the regression proof: 20/20 before and after the Task 6 rewire, unchanged, which (running in workerd against a real miniflare D1) pins the exact rejection statuses, content-types, bodies, all six security headers, the local-http exemption, and the three log reasons. The `delivery-*-split` parallel-load flake surfaced once during Task 5 and cleared on a full-suite re-run, the known transform-contention flake, not a regression.

**Review gate ran clean.** The `code-simplifier` made no change (the registry is an intentionally verbatim data table; the renderer and `CairnError` are already tightest-form). `web-auth-security-reviewer`: no Critical/Important. It confirmed the gate order and pre-`resolve()` placement are identical, the status/headers/body are byte-for-byte preserved, the `ctx.url!` assertion moved an existing precondition rather than weakening it (the one https caller always passes `{ url: event.url }` after reading `event.url.protocol`), and no request-reachable new throw or weaker-response path exists (`id` is a hardcoded literal at every call site). `cloudflare-workers-reviewer`: no Critical/Important. It confirmed the `Response`/`URL` construction is workerd-valid, the new module-level state is read-only (`REGISTRY`, `REASON_CONDITION`), the synchronous rejection path has no eval/dynamic-import/await, and the type-only `CairnLogEvent` import plus the not-yet-called `CairnError` add no Worker edge weight.

**Live admin smoke: skipped, judged not proportionate** (the same call plans 1 and 3 made). The only change is a behavior-preserving re-home with no rendering change and no new runtime behavior, and the workerd integration suite already pins the exact responses.

**Two carry-forwards (recorded, not fixed).** (1) Both reviewers independently noted `REGISTRY` is a mutable `Record` by type, relying on convention to stay read-only; `as const satisfies Record<string, CairnCondition>` or `Object.freeze` would enforce it. Defensive only, left to a future diagnostics touch when the registry grows. (2) `renderConditionResponse` enforces the registry/renderer 1:1 with two separate structures (the `condition(id)` membership assert plus the `switch` `default` throw), so an id in one but not the other is caught but by two mechanisms; a single coverage test asserting `REASON_CONDITION` values all both resolve and render would tie them. The Task 5 test already asserts the resolve half. (3) Pre-existing, not introduced here: nothing asserts the ordering between the https branch and the csrf branch (an http admin form POST with a bad token should get the 400 https page, not the 403); the refactor preserves the code order, so it is an untested invariant, not a regression.

**Next:** Pass 2 (the email-delivery runtime arm), which lands `CairnError`'s first throw-site. Design in `docs/superpowers/specs/2026-06-08-cairn-email-delivery-and-environment-preflight-design.md`.
