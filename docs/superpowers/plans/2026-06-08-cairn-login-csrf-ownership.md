...
## Post-mortem (executed 2026-06-08, landed on `main` as `0.35.0`, unpublished)

All eleven tasks landed subagent-driven on `main`, one `cairn-implementer` per task, verified commit-by-commit. Tasks 5 and 10 ran on Opus for the security and breadth judgment; the rest ran on the Sonnet default. Thirteen commits total: the eleven task commits `93774a6..5514988`, a simplifier commit `cbd8b31`, and a svelte-review fold-in `b165df3`.

**What was built.** cairn's auth guard is now the single CSRF authority. A consuming site disables SvelteKit's global `checkOrigin`, and the guard enforces two rules: every unsafe `/admin` form POST carries a valid `__Host-cairn_csrf` double-submit token, and every non-admin unsafe form POST keeps a strict `Origin` check. The token is issued lazily and stably by the login, confirm, and admin-shell loads (it reuses the cookie when present, so a second tab matches), rendered by a shared `CsrfField` component, and validated centrally with a constant-time compare. A failed admin check serves a branded 403 built through a new shared static-page shell extracted from the HTTPS-required page. The CSRF cookie is session-scoped, HttpOnly, SameSite=Strict, and `__Host-` on https.

**Verification (first-hand at the tip `b165df3`).** `npm run check` 836 files 0/0; `npm test` 128 files / 796 tests exit 0; `check:docs`, `check:reference`, `check:package` each exit 0; `examples/showcase` production build exit 0. The workerd integration suite proves the three guarantees end-to-end against real miniflare D1: an admin POST with a valid token and no `Origin` passes and sets `locals.editor`, an admin POST with no token gets the branded 403 and never resolves, and a non-admin cross-origin form POST is rejected.

**The plan gap the execution caught (Task 10).** The plan enumerated the authed POST forms but missed two real admin mutation forms, `DeleteDialog.svelte` (`?/delete`) and `RenameDialog.svelte` (`?/rename`), which render as descendants of `EditPage` and `ConceptList`. Both now carry a `CsrfField`. Without that, those two mutations would have failed closed at runtime (the branded 403) the moment a site enabled cairn's CSRF ownership. The implementer found them by surveying the real markup rather than trusting the plan's list, the intended "trust the source" discipline for a broad task.

**Review gate.** The simplifier shared one branded-page response builder in the guard (`cbd8b31`). The `web-auth-security-reviewer` returned no Critical or Important: it confirmed the `__Host-` cookie binding and fixation resistance, the constant-time compare with empty-token rejection, the body clone leaving the action's body readable, and the guard ordering that gates every unsafe `/admin` POST before `resolve()`. The `daisyui-a11y-reviewer` cleared both branded pages at WCAG 2.2 AA in light and dark with margin. The `svelte-reviewer` raised one Important: `setContext` captured `data.csrf` once at init, so a future mid-session token rotation would leave descendant forms posting a stale value. The fold-in `b165df3` hands a live getter through context and drops the cosmetic `untrack`, making descendant-form correctness structural rather than comment-dependent.

**Proportionality call on the live smoke.** A full wrangler-dev CSRF browser round-trip needs a consumer with `checkOrigin: false` and the real admin mounted, which the showcase is not (it uses its own fake handle with the framework check on). The workerd integration tests exercise the guard's two rules in the real runtime, so the live smoke was judged not proportionate here and the real-runtime CSRF verification rides the first site retrofit, where `checkOrigin: false` is actually wired. This matches the call plans 1 and 3 made.

**Carry-forwards (recorded, not fixed).** (1) Bare `<CsrfField />` depends on an ancestor `setContext`, an undiscoverable coupling: a new admin form rendered outside `AdminLayout` (the coming `CairnExtension` seam) would ship an empty field and surface only as a submit-time 403. A dev-time warning when both the prop and the context getter are absent would make it loud. (2) The CTA hover on the branded pages has an ungated `transform` transition; a `prefers-reduced-motion` guard is optional polish (AAA, not required for AA). (3) `LayoutData.csrf` admits `''` for non-route callers; behavior is fail-closed either way, a clarity nit only.
