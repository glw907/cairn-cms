...
**Built.** All 11 tasks. `fieldset()`/`fields.*` is the one live field system; v1 (`defineFields`, `FrontmatterField`, `validate.ts`) is removed. `ConceptDescriptor.fields` is `NamedField[]` (the record key re-attached in `normalizeConcepts`), and the descriptor also carries `schema: Fieldset` so `editLoad` resolves `default: 'today'` against a request clock. The editor gained arms for number, select, url, email, datetime, and a unified multiselect (closed checkboxes / creatable tag input) gated by the shared `isClosedMultiselect` predicate. The showcase migrated and added a `status` select. Docs rewritten off `defineFields`; 0.69.0 CHANGELOG + upgrade entry with the breaking `Consumers must:` block.

**Verified (evidence).** `npm run check` 0/0 (1178 files); `npm test` exit 0 (2513); `check:comments` OK; the four doc gates (`check:reference`, `check:reference:signatures`, `check:package`, `check:docs`) and `check:version` (minor) green; the from-scratch consumer-build showcase e2e 36/36 (the `status` select round-trips create → save → reload). code-simplifier (one refinement), `svelte-reviewer`, and `daisyui-a11y-reviewer` all clean; two low-severity a11y/polish items applied. The `/admin` surface is covered by the browser component tests, the workerd integration layer, and the full editor round-trip in the e2e, so the wrangler-dev manual smoke was treated as redundant.

**Decisions locked.** `ConceptConfig.schema` kept (no `defineConcept` yet); `NamedField[]` internal shape; the `Infer` reclaim dropped, `InferFieldset` the one extractor (module-local `Infer<R>` renamed `InferRecord`); datetime naive-local minute-precision; `placeholder` added to `MultiselectField`; the SEO guard ported into `fieldset()`; create-form prefill wired at `editLoad`.

**Execution note.** Planned from a workflow (8 seam-specs → 3-lens adversarial hazard sweep → Opus synthesis). The hazard sweep's central finding reshaped execution: the cutover is one atomic compile unit, so the atomic core (tasks 4-9) ran as a single `cairn-implementer` dispatch gated once at the end, not six independently-gated tasks. The datetime deviation (no `serializeMarkdown` special-case) was verified correct: minute precision dodges the YAML 1.1 timestamp resolver, so js-yaml round-trips it as a quoted string.

**Engine / DX deficiencies surfaced (carry-forwards).**
1. **Round-trip fragility for any new field type whose editor format differs from its stored/parsed format.** datetime nearly shipped a silent data-loss (the `datetime-local` input blanks an ISO-with-seconds/`Z` value, dropping the key on re-save). Caught by the hazard sweep, not by a gate. Carry-forward: make a frontmatter round-trip property test (`form → frontmatterFromForm → serializeMarkdown → parseMarkdown → formValues`) a standing requirement for every new field type, especially **references** next (they resolve to a permalink + summary at delivery, a richer round-trip).
2. **One shared test fixture couples ~24 test files to the field-system shape,** so a field-contract change is all-or-nothing (no incremental green). It made this cutover an atomic dispatch. Acceptable here, but the next field-contract change (references adds a field type) hits the same wall; plan it as atomic from the start.
3. **Declared-but-unread descriptor flags are a latent trap.** `taxonomy` on `MultiselectField` is still declared-only with no consumer (reserved); `creatable`/`placeholder` only gained consumers this pass. A reviewer can assume such a flag gates behavior when it does not. Carry-forward: when references lands, wire or explicitly document each new descriptor flag's consumer, and keep `taxonomy` flagged as reserved until the taxonomy/tag-index work consumes it.
4. **New field arms render behind the Details slide-over (closed by default).** Correct for the quiet surface, but as the field vocabulary grows (references, object/array) the default-closed panel buries more. Worth revisiting the Details-open heuristic when the field count per concept rises. Also a test-authoring gotcha: any field e2e must open Details first (this pass's e2e initially missed it).
5. **`ConceptDescriptor` grew `schema: Fieldset`** to reach `initialValues` at request time. Minor surface growth; the alternative (precomputed `initialValues`) cannot resolve `'today'` at build time. Revisit if the descriptor accumulates more request-time needs.
6. **Worktree DX friction (not engine):** under the symlinked-`node_modules` worktree, the showcase `npm run check` shows ~12 dependency-`.d.ts` errors from a dual vite/kit install collision (showcase-local + symlinked root). A worktree-only artifact (CI's real checkout is clean), but it muddies the local consumer typecheck signal; the consumer-build proof had to come from the e2e build, not the typecheck.
