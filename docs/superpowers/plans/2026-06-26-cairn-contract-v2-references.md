...
**Built.** All 16 tasks plus a review-gate fix-up round. The frontmatter reference field is live, single `reference()` and many `array(reference())`: the descriptors + inference + the shared canonicalizer (`frontmatter.ts`), the form decode and the two-sided round-trip, the schema-driven edge extractor (`extractReferenceEdges`), the manifest `references` edge array + `inboundReferences` (pair-matched) + the `mediaRefs`-mirrored back-compat, the strict fail-closed cross-branch `buildReferenceIndex`, the `verifyReferences` build gate wired into the vite virtual module, the surgical `rewriteFrontmatterReference` (built on the extracted `frontmatter-region.ts` helpers), the `EntryPicker`/`ReferenceField` editor arm, the save-time warning, the rename repoint + third-party-branch refusal, the fail-closed delete gate, the `resolveReferences` read-model resolution at the site-resolver layer, and the showcase migration + e2e.

**Verified (evidence).** `npm run check` 0/0 (1192 files); `npm test` exit 0 at **2651** (from a 2513 baseline, +138); `check:comments` + the four doc gates (`check:reference`, `check:reference:signatures`, `check:docs`, `check:package`) + `check:version` (minor → **0.70.0**) all green after the fix-up; the from-scratch consumer-build showcase e2e green (37 at Task 15; re-run against the fixed dist at consolidation). `code-simplifier` (3 refinements) and four review subagents (`svelte`, `web-auth-security`, `cloudflare-workers`, `daisyui-a11y`) ran; their findings are folded (below). 16 task commits + 1 simplifier + 1 docs + 2 fix-up.

**Decisions locked.** Single and many ship together (a reference-constrained `array` now, the constructor's final general signature). Taxonomy deferred and queued. The manifest carries a separate `references: {field,concept,id}[]` array (not merged into `links`). The cross-branch index is strict/fail-closed; rename refuses a third-party open-branch inbound (the predicate gates `origin.kind === 'branch'` first) and repoints `main`; delete fails closed (503 on a strict-build throw). Resolution lives at the `site-resolver` layer (cross-concept reach), not the per-concept index. The rewriter re-quotes a YAML-significant `newId` AND extends the splice over an existing quote pair (the quoted-old case). One shared canonicalizer (Date→UTC-slice, bare-scalar→single-element) across validate/extract/formValues. `verifyReferences` runs inside `virtualSource`'s verify expression (where `built` exists), the only integrity authority (no prerender backstop).

**Process notes (the load-bearing lessons).**
- **Incremental, not atomic — confirmed.** Adding `reference`/`array` to the union compiled through the default-bearing switches, and keeping the shared `_content-fixture.ts` reference-free with dedicated fixtures let every task gate green on its own. The cutover's "plan it atomic" prediction (its carry-forward #2) assumed a delete-and-retype; an additive type is incremental.
- **The pre-plan hazard sweep and the pass-end reviewer fan-out are complementary, and BOTH were necessary.** The 5-lens sweep (21 confirmed findings) reasoned about the design and caught the `verifyReferences` virtual-module wiring, the resolution-layer blocker, the rename refuse-predicate, and the rewriter re-quote, but its rewriter and round-trip lenses dropped on a connection error and had to be re-run (the re-run caught the two Task 2 rewriter blockers). The pass-end reviewer fan-out then read the BUILT code and caught **three blockers the sweep, the implementers' self-reviews, the e2e, and 2592 tests all missed**: two content-corruption bugs in the commit pipeline (the rewriter quote-NESTING on an already-quoted old id, the inverse of the sweep's bare-old case; and the rename double-write clobber for an entry that both body-links and references the target) plus the reference-only-edit-unsaveable UX bug. Both corruption bugs lived in code paths the tests did not exercise (a quoted old value; a combined linker), exactly where a fresh-eyes adversarial read on the real code pays and where reasoning about the design does not reach.
- **The empirical wiring proof is the gold standard.** Task 8's implementer proved the build gate is load-bearing by deleting `verifyReferences(built)` from the resultExpr (the dangling test went red) and restoring it (green), rather than trusting the wiring.
- **Worktree e2e infra:** the vite-integration test must root its temp project inside the worktree (the `@glw907/cairn-cms` self-symlink does not resolve from `/tmp`); gitignored as `.cairn-vite-test-*/`.

**Carry-forwards.**
- The taxonomy / tag-delivery pass (queued in ROADMAP's Next tier; consumes the reserved `taxonomy` marker).
- The body-link cross-branch delete follow-up (queued; lift body-link delete protection from main-only to the strict cross-branch gate references now use).
- The `delivery-*-split` / `delivery-data-dist-spawn` cold-import tests flake under full-suite load (a test-infra watch item; logged to the friction log).
- Defensive `never`-exhaustiveness assertions on the field-system switches, for the phase-2 `object` type (noted for the phase-2 plan).
