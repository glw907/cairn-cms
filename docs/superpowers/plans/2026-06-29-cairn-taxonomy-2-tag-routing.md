...
- [ ] **Step 3: Wire the catch-all.** `+page.server.ts` calls `routes.resolveRoute({ url })`, throws `error(404)` on undefined, returns the discriminated data. `+page.svelte` switches on `data.kind` to render the entry, the tag index (links via the engine-built paths), or the tag archive.
...
- [ ] **Step 5: Run the e2e** (`CI=1 npm --prefix examples/showcase run test:e2e`). Confirm green. Confirm the prerender enumerates the tag paths (Task 3's `entries()`).
