...
Phase 6 is complete (2026-06-30). The admin `retiredTokenBudget` reached its terminal floor, **0**.
The sweep's retired-token goal is met: the admin component tree holds zero `var(--color-muted)` /
`var(--color-subtle)` bracket references.

**What was built.** Four retirement clusters, each a `cairn-implementer` (Sonnet) dispatched through a
single background Workflow that ran them as a strict in-order ratchet (they share the budget file, and
clusters 1-3 edit the same `CairnMediaLibrary.svelte`, so a parallel fan-out would have clobbered).

- **Cluster 1** (slide-over model 1 + safe-delete model 2, lines 1634-1911): 22 tokens, including the
  `:1651` subtle and the `:1841` delete-in-use false-arm. Budget 125 -> 103.
- **Cluster 2** (replace model 3 + push-alt model 4, lines 1917-2439): 40 tokens, including the `:1966`
  subtle, the `:1988`/`:1993`/`:2353` card-border entangled sites, and **the one non-trivial site**, the
  `:2322` strikethrough. Budget 103 -> 63.
- **Cluster 3** (bulk-delete model 5 + orphan-scan model 6, lines 2448-2974): 43 tokens, the
  `:2570`/`:2874`/`:2877`/`:2951` entangled sites; `CairnMediaLibrary.svelte` fully swept (0 brackets).
  Budget 63 -> 20.
- **Cluster 4** (the four small media files): `MediaHeroField` (14), `MediaFigureControl` (4, the `:179`
  roving-radio false-arm), `MediaCaptureCard` (1), `MediaInsertPopover` (1). Budget 20 -> **0**.

Then the slide-over visual baseline (Task 5): two new snapshots (`admin-media-detail-{light,dark}`)
capturing the non-modal detail `region`, opened by clicking the first grid `option`.

**The `:2322` decision: option (b), confirmed.** The push-alt "alt was" diff row struck through old alt
text with `decoration-[var(--color-muted)]/55` (a text-decoration-COLOR at 55% opacity), the one form the
named role layer cannot express. It was rewritten to the sanctioned currentColor idiom
`decoration-[color-mix(in_oklab,currentColor_55%,transparent)]` (the `/55` baked into the mix), the same
form `TidyReview.svelte:363` and `EditPage.svelte:1841/1946` already use. No new `@utility`, no
frozen-layer change, no test touch. The recorded drift: the span sets `text-base-content`, so the
strikethrough tint shifts from muted-at-55% to base-content-at-55% on that one diff row, a near-identical,
intended change. It is not separately baselined: rendering an "alt was" row requires a four-step
interaction (open slide-over, open the alt dialog, run the preview, check the overwrite box) on seeded
media that carries a custom-alt placement, disproportionate to a decorative strikethrough tint. The
daisyui-a11y review confirmed it is decorative under WCAG 1.4.1 (the `text-primary` "after" value plus the
arrow icon carry the before/after meaning, not the line tone).

**What was verified.** Per-cluster: the full gate green at each ratchet (`check` 0/0, `npm test` exit 0
with 2867 tests, `check:custom-surface` PASS both trees at the cluster's measured budget). The whole-tree
retired-token count measures 0; the diff is a symmetric 125/125 swap. Spot-checked every entangled
landmark: `:1841` keeps `cairn-error-tint`/`cairn-error-ink`, the card-border sites keep
`--cairn-card-border` (incl. the `/70` variant), `:2874` keeps `--cairn-code-chip`, and
`MediaFigureControl:179` keeps `bg-primary/10 text-primary` on its true arm. The slide-over baseline
generated clean and the existing 14 snapshots regenerated byte-identical (the zero-pixel proof); the
showcase admin-visual e2e passed 16/16 on a fresh `CI=1` build.

**Review gate, all clean.** code-simplifier: no-op (the diff is a pure 125/125 token rename with nothing
accidental). svelte-reviewer: no findings; reactivity, markup, roles, and the six a11y models intact.
daisyui-a11y-reviewer: ship it; contrast roles preserved (the named utilities resolve to the same
`--color-muted` / `--color-subtle` oklch literals, locked by `role-layer-contrast.test.ts`), the six a11y
models structurally byte-identical, the `:2322` shift decorative and acceptable, no DaisyUI 5.6 misuse.

**Decisions held (do not relitigate).** The six a11y models are color-only edits, untouched: the
non-modal slide-over `role="region"` (no focus trap), the three `alertdialog`s, the deliberate
`role="dialog"` push-alt, and the orphan list as plain labelled native checkboxes (not a listbox). No
native-primitive adoption, no `@layer` fold (`cairn-admin.css` untouched, `componentsLayerCap` 14), no
coupled-test migration, no presence-only a11y test owed (the one gap, `MediaPicker.test.ts:90`, was
hardened in Phase 5). The CodeMirror content theme stays the walled hard floor.

**The terminal floor.** Budget 0 now blocks any new muted/subtle bracket token anywhere in the admin
tree. The residual custom surface is only the Tier-2 sanctioned set (the a11y inks, the elevation pair,
the editor system, the embed-anywhere infrastructure, the two unlayered workarounds), owned on purpose
and documented in the ledger. The admin idiomatic re-expression sweep (Phases 0-6) is complete.
