...
- Modify: `.github/workflows/create-site.yml:105-107`, `:131-141` (assert `.github/workflows/check.yml`
  ...
  handover text names `npm run check:cairn` and the workflow), `docs/extend/what-the-scaffold-wrote.md`
  (which also states that `check:cairn:rendered` ships in the site's scripts and needs a browser
  the workflow never installs), `docs/admin/create-your-site.md` (the prose around the three
  quoted blocks), `CHANGELOG.md`
...
- [ ] **Step 1: the failing assertion first.** The workflow-file assertion fails on the current
  scaffold.
- [ ] **Step 2:** the CI assertions, the handover text, the two docs pages.
