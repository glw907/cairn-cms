...
opt-in (Geoff, 2026-09-21). Every other task is an Agent-tool chain: `cairn-implementer`, then
...
**Token ceiling:** 7M, flag at 5.6M. **Segments and checkpoints:** fourteen tasks in four
segments, 1 to 5, 6 to 8, 9 to 10, 11 to 14; a checkpoint at each boundary, written to the ledger
at the foot of this file. The one attended stop is the owner sitting at the end of task 8, which
coincides with the second checkpoint; every other question batches there or at a boundary.
...
platform reasoning, `diff-reviewer`), and tasks 11 and 3's Go edits, which change tests that
...
1. The whole doctor retirement is merged on `origin/main`: `src/lib/doctor` is gone,
   ...
   bin is gone from `package.json`, and `cairn doctor` exists in the cobra tree.
...
6. The inventory matches the day: the unique `docsAnchor` count in
   `src/lib/diagnostics/conditions.ts` (20 at plan time, after `declare-the-media-bucket-binding`
   joined), the three anchors in `tool/internal/health/fixes.go`, the readers of
   `tool/docs/credentials.md` and `tool/docs/tripwire.md` under `tool/` (listed in task 11), and
   `PAGE_FLOORS` in `scripts/checks/transcript-blocks.mjs`. A different count or reader list is
   recorded in the ledger and the affected task text amended before dispatch.
7. `CAIRN_GH_READ_TOKEN` is unexpired (it expires 2026-10-19) and the four sites answer
   `cairn health` from this workstation with the stored `CAIRN_*` values; a failure here is a
   credential task for Geoff, not a docs finding.
...
- Every example and transcript uses the substitutes `example.org`, `example-org/site`, and
  `<account-id>`; no page or fixture carries the owner's site names, repositories, or Cloudflare
  account id. The fixture manifest applies the substitution at capture (task 5).
...
| `tool/testdata/transcripts/README.md`, `*.txt` | 5 | the tool captures and their manifest |
...
- the assertions the drift tests make about page content, verbatim: in `permissions_test.go`,
  the `## Token scopes` and `### GitHub` headings it finds and how it matches a
  `permissionTable` row's label to a bullet; in `messages_test.go`'s budget-docs test, the wire
  value and the cap it asserts in `tripwire.md`;
...
- the four sites' worker names and which of them `cairn health` reaches with the stored values.
...
accepted `troubleshooting.md` is the symptom-row exemplar. The dated record gains one appended
note pointing at the register. The friction log's open section notes the amendment in one line.

**Acceptance:** the admin section is the only track section changed; the record's body above the
note is unchanged; the docs gate is green; `diff-reviewer` accepts.
...
- `transcript-blocks.mjs` resolves a marker against two roots: the scaffolder's, unchanged, and
  `tool/testdata/transcripts/`; a marker names which by its leading path segment. `PAGE_FLOORS`
  gains `docs/admin/install-cairn.md: 1`, `check-your-credentials.md: 1`,
  `schedule-a-check.md: 1`, and keeps the two existing floors;
- `check-facts.mjs` accepts `[vendor-figure: as of YYYY-MM-DD]` as a distinct tag whose bullet
  must carry a `Source:` that is an absolute vendor URL, and rejects it without a date or with
  a repo path; `docs/internal/facts/README.md`'s tag vocabulary documents it with the rule that
  a page prints the figure with its as-of date;
- the fixtures README's "Deliberately unconsumed" heading declares the scaffolder fixtures the
...
`check:transcripts` and one naming a missing file fails naming the root; a bullet
`[vendor-figure: as of 2026-09-21]` with a vendor URL passes `check:facts` and one with no date
fails; the docs gate is green; `diff-reviewer` accepts.
...
sending domain. The two dashboard steps the scaffolder narrates (GitHub push, Cloudflare
connect) are done through `gh` and the Cloudflare API, never the dashboard. The ledger lists:
the repository, the Worker, the D1 id, the Builds connection, and the teardown commands for
each.

**Acceptance:** `cairn adopt` then `cairn health` against it exits non-zero with the sending
domain and origin checks failing and every other check observable; the resource list is in the
ledger; nothing under `docs/` changed. Task 14 tears it down.
...
class (a healthy site, the scratch site), the date, and which page consumes it. The set covers:
...
One Sonnet read per page (twelve, dispatched in parallel, files disjoint by section), then one
`claude-opus-5` dispositions read over all twelve. For the nine rebuilt pages the old page is the
...
The page-chain workflow with one page. Then the anchor landing test.
...
list, 20 at plan time, plus the three `fixes.go` anchors, all within that list) and each keeps
its heading text. The B2 harvest's health, adopt, and machine-surface bullets are the tool
...
manifests for this page; its bullets re-keyed; no page exemplar (task 2's rule); the error-tier
Vale rule list.
...
**Acceptance:** the docs gate is green, `check:readiness` included; `make -C tool check`
(`TestFixLineAnchorsResolveInDocs`) is green by absolute worktree path; the workflow record says
accepted; the six cold reads land; no second `fix`, or it is with the conductor.
...
where the site registry lives, per platform, exact paths; `## Token scopes` with `### GitHub`
and `### Cloudflare` subsections whose bullets carry the `permissionTable` labels task 1
recorded, verbatim, because `permissions_test.go` reads this page after task 11; the success
signal (`cairn auth check` green, its transcript); and each failure the reader can hit ending
in a wait, act, or ask step. The macOS and Windows sections carry the not-verified-by-a-person
note. It does not cover scheduling or health.

**Inputs the drafter receives:** as task 7, plus the task-guide anatomy and no page exemplar.

**The cold reader, after the chain accepts:** one `claude-opus-5` agent in a fresh distrobox
container with the page and a terminal only, held to task 2's ceiling, with a throwaway pair of
tokens minted for it and revoked after; it follows the page literally, runs every command as
printed, and logs every point where it had to infer, ask, or stop. The log is written to
...
- `schedule-a-check.md` (new). The reader's job: run `cairn health --quiet` on a schedule and be
  ...
  shell profile and the environment-first order. The page carries the `not-run` wire word and
  the sweep cap in seconds verbatim from the manifest, because `messages_test.go`'s budget test
  reads this page after task 11. Alerting is one section: choose the threshold, alert once,
  never a credential. macOS and Windows carry the note.
...
headings must keep resolvable, or the brief names the `LEGACY_PATH_MAP` row), the cost lines,
and what the page does not cover.

**The cold readers:** one `claude-opus-5` agent per page, fresh container, page and terminal
only, the ceiling in the prompt; local and read-only commands run for real, cloud steps run
against the scratch site only where the command reads (health, logs, holds), dashboard steps
reasoned aloud. Each stall is a finding through one redraft from the fact read onward.

**Acceptance:** the docs gate is green apart from `check:arm-indexes` for the two new pages,
which task 10's README closes; every workflow record says accepted; every cold-reader stall is
resolved or with the conductor.
...
- `what-to-run-and-when.md` (rebuilt). The scheduled run bullets from the harvest; the target
  stack section unchanged in substance (its `check:target-stack` source).
- `before-you-start.md` (rebuilt). The install and credentials bullets; every cost from a
  `[vendor-figure]` bullet with its as-of date, before the step that incurs it; the free-until
...
still resolve every link into the arm (task 1's inbound list), or the `LEGACY_PATH_MAP` row is
in place.
...
- `permissions_test.go` reads `docs/admin/check-your-credentials.md` through
  `providers.RepoRoot()` and its assertions pass against the new page unchanged in meaning;
  `messages_test.go`'s budget-docs test reads `docs/reference/cli-cairn-exit-codes.md` and
  `docs/admin/schedule-a-check.md`; the comments in `permissions.go`, `messages.go`, and
  `health_quiet_test.go` name the new paths;
- `tool/README.md`'s two links point at the cairn.pub URLs of the two pages, with the repository
  path in parentheses;
- `tool.yml`'s two `paths` lists carry both admin pages;
...
`git grep -n "docs/credentials.md\|docs/tripwire.md" -- tool/ .github/` returns only
`tool/docs/release-candidate-notes.md`, `tool/docs/design/`, `tool/CHANGELOG.md`'s released
entries, and the new stub; `diff-reviewer` accepts against this list.

### Task 12: The literal walk and the platform reasoning

Two dispatches, independent.

**The literal walk:** one `claude-opus-5` agent in a fresh distrobox container on this
workstation, with the four pages `install-cairn.md`, `check-your-credentials.md`,
`is-it-working.md`, and `schedule-a-check.md`, a throwaway token pair, and the scratch site's
name; nothing else. It installs, stores credentials, adopts and checks the scratch site, and
enables the systemd user timer, every command as printed, every success signal asserted as the
page states it, and it logs each deviation. The timer captures from task 5 are taken here. The
container and the tokens are destroyed after.
...
**Acceptance:** the walk reports every command ran and every signal appeared, or each deviation
is resolved by one redraft and one re-walk; the reasoning returns no unresolved finding; the
container is gone.
...
frozen contracts, and the exit-code and `--json` exclusion from the arm.
...
- `CHANGELOG.md`, under `## Unreleased`: the admin arm redrafted with three new pages, the
  ...
  what someone who linked to the old paths does;
...
- `docs/HISTORY.md` gains the pass entry, with what a later pass would be wrong to rediscover:
  the two Go drift tests that pin headings and words on the admin pages, the fixture root
  split, the vendor-figure tag, the stall-log rulings, the mining measurements, and the cold
  reader method;
