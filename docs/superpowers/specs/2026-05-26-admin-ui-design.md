...
- **Sequence:** its own design round (reshapes the adapter contract + admin mounting). Naturally **follows the theme-architecture extraction** (which establishes the registry/adapter aggregation cairn-core needs to also aggregate extensions). Detail: `docs/creating-a-cairn-site.md` → "The canonical extension contract".
