<!-- @component The Foxi port's public chrome: a sticky NavigationBar, a growing main, and a
     SiteFooter in a (site) route group. This port mounts no /admin, so the group wraps every
     route. Both stylesheets link by their ?url-resolved URL, the chassis pattern that also lets
     an editor preview frame link the same asset in a site that keeps an admin. -->
<script lang="ts">
...
<div class="cairn-site-shell bg-base-100 font-body text-base-content">
  ...
  <main id="main" tabindex="-1" class="cairn-site-main">
    {@render children()}
  </main>
