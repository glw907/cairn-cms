...
export const handle: Handle = async ({ event, resolve }) => {
  if (FAKE) {
    ...
    if (isAdmin || isMedia) {
      ...
      // action under /admin and the delivery route under /media. ANTHROPIC_API_KEY is a dummy presence
      // flag: the tidy action refuses before building a client when it is absent, so the value is set
      // even though the fake client (fake-anthropic.ts) never reads it.
      event.platform = {
        env: {
