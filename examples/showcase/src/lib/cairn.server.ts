...
// the e2e's VITE_CAIRN_E2E=1 build), and the anthropic import is dynamic, so a default production
// build folds the whole block out (DCE), keeping the dev package's bypass barrel out of the deployed
// bundle. A real deployment leaves anthropic unset: the content routes build the real Anthropic
...
export const admin = createCairnAdmin(runtime, { anthropic });
