...
// five fake-backend factories must resolve from the package root, proving the re-exports and the
// workspace link are wired before any consumer imports them.
test('the dev package re-exports the five fake-backend factories from its root', () => {
  expect(createDevBackend).toBeTypeOf('function');
  expect(createFakeAuthDb).toBeTypeOf('function');
  expect(createFakeAppDb).toBeTypeOf('function');
