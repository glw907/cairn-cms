...
// allowlist matcher all live in src/lib/audit/rendered.ts and its rendered/ siblings now, exercised
// by the audit's own fixture suite. What remains is the one thing the engine deliberately does not
// do, since its own page list is a consumer's configured routes: enumerate the showcase's public
// pages from its sitemap, so a new page joins both gates without either script being edited.
//
