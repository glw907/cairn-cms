 ...
 * time, so a call made after a gated batch's manifest hash (which covers every file under
 * `PROMPTS_DIR`) pins the prompt bytes never returns a copy cached before that pin.
