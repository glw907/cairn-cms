...
// save-state, the lifecycle actions), so the desk's controls have to render up in CairnAdminShell's
// one topbar rather than in a second header of their own. CairnAdminShell owns a holder and provides
...
export interface TopbarHolder {
  desk: Snippet | null;
  /**
   * True while the document is in zen: CairnAdminShell drops the whole topbar element so the band
   *  slides away (the desk's three clusters include CairnAdminShell-owned chrome, the drawer toggle and
