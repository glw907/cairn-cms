...
export const REGISTRY: Record<string, CairnCondition> = {
  ...
  'config.dependency-floors-unmet': {
    ...
    why: 'The lockfile resolves svelte or @sveltejs/kit below the range the engine declares as a peer. Consumer sites compile the shipped .svelte sources, so a below-floor compiler bites silently at build time; the svelte floor is ^5.56.10, the version cairn develops and tests against, and it sits above the 5.56.1 release that miscompiles parenthesized boolean groupings.',
  ...
  'auth.identity-unresolved': {
    id: 'auth.identity-unresolved',
    severity: 'blocker',
    title: 'Sign-in gate did not confirm who you are',
  ...
  'auth.identity-unknown': {
    ...
    severity: 'warning',
    title: "You're not on this site's editor roster",
    why: "The site's identity gate confirmed who you are, but your email is not in cairn's editor roster, so you cannot sign in to the admin.",
    remediation: 'Ask an owner to add your email to the roster through Manage editors.',
    docsAnchor: 'is-it-working.md#youre-not-on-this-sites-editor-roster',
    logEvent: 'auth.identity.unknown',
  },
