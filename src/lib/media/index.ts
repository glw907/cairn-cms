...
// so a plain-Node tool or a build step can import it. Anything proposed here must be node-safe pure
// projection over the media surface; the manifest CRUD, the hashing and naming helpers, the raw
// transform-URL builders, and the preview-only resolver are the engine's own ingest/admin
// internals, every real caller reaches them by relative import, never through this barrel, so they
...
// `AssetConfig`'s canonical home is `.`; a recorded R4 re-export here because `normalizeAssets`'s
