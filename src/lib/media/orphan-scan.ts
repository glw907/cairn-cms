...
// the screen renders: the purgeable byte-rows (stored R2 objects with no manifest row) and the
// read-only broken-reference rows (manifest rows whose bytes are gone). It only projects; no path
// here reads R2, the manifest, or git. The module is engine-internal and on no public subpath.
 ...
 * object) is skipped. `brokenRefs` come from `reconcile.missingObjects`: each hash carries its
