...
export function variantUrl(publicPath: string, spec: VariantSpec): string {
  ...
  // media/config.ts, now Object.frozen) and validateVariant was retired alongside the site-declared
  // `variants` field (ruling 4, 2026-09-01). Reintroducing a caller-supplied VariantSpec (a site
  // declaring its own preset again) reopens comma/slash injection into the transform URL's options
  // segment; re-add a value check on gravity and format before that seam returns.
