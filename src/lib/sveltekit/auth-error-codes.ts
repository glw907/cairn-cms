...
// sender) toward a client bundle to get it. The package declares `sideEffects` for `.svelte` and
// `.css` only, so a bundler can drop an unused `.ts` either way; this module makes that
// independent of the bundler's tree-shaking rather than reliant on it.
//
