// cairn-cms: the entry destructive and revert clusters (deleteAction, listDeleteAction,
// renameAction, revertAction). The read cluster (createAction, editLoad, historyLoad) lives in
// content-routes-entry-read.ts, and the write cluster (saveAction, publishAction,
// publishAllAction, discardAction) lives in content-routes-entry-write.ts.
...
//
// These handlers folded in here from content-routes-core.ts, since retired.
...
import { parseMarkdown, serializeMarkdown } from '../content/frontmatter.js';
...
/**
 * Build the entry destructive and revert clusters, closed over the shared content-routes
 *  context: delete, rename, and revert. The read cluster (create, edit, history) lives in
