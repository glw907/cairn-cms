...
// machinery the plugin shares with the cairn-manifest bin, including the site-facts.json contract
// cairnManifest's buildStart also verifies, lives in `./internal.js`, which that bin and its unit
// tests import by relative path, never through this subpath: a build-time helper that is not
// itself a Vite plugin belongs internal, not here.
//
