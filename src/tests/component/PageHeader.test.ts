...
describe('PageHeader', () => {
  ...
  describe('the header-stack gaps and the mobile action width (compiled sheet)', () => {
    ...
    // both widths (the inner eyebrow/h1/meta stack does not change orientation across that
    // breakpoint, only the outer header row/column does), which is why one band checks both.
    it.each([
      ['the desktop width', 1440, 900],
      ['the mobile width', 390, 700],
    ])(
