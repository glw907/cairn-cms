...
describe('FieldInput dispatches each field type to its arm', () => {
  it('renders a plain text input (no type attribute) carrying the form name and value', async () => {
    const field = { type: 'text', name: 'title', label: 'Title' } as NamedField;
    await render(FieldInput, { field, frontmatter: { title: 'Hello' }, ...shared() });
  ...
  it('renders a textarea with the value as its content and the configured rows', async () => {
    const field = { type: 'textarea', name: 'summary', label: 'Summary', rows: 5 } as NamedField;
    await render(FieldInput, { field, frontmatter: { summary: 'A paragraph.' }, ...shared() });
  ...
  it('renders a number input with min, max, integer step, and the native required attribute', async () => {
    const field = { type: 'number', name: 'count', label: 'Count', min: 1, max: 9, integer: true, required: true } as NamedField;
    await render(FieldInput, { field, frontmatter: { count: 4 }, ...shared() });
  ...
  it('renders a select with a leading empty option and the schema options, marked required', async () => {
    const field = { type: 'select', name: 'tone', label: 'Tone', options: ['note', 'warn'], required: true } as NamedField;
    await render(FieldInput, { field, frontmatter: { tone: 'warn' }, ...shared() });
  ...
  it('renders a date input that always carries a hint paragraph (the publish-clarity default)', async () => {
    const field = { type: 'date', name: 'date', label: 'Date' } as NamedField;
    await render(FieldInput, { field, frontmatter: { date: '2026-05-01' }, ...shared() });
  ...
  it('renders a boolean checkbox reflecting the frontmatter truthiness', async () => {
  ...
  it('renders a closed multiselect as a fieldset of checkboxes, one per option', async () => {
    const field = { type: 'multiselect', name: 'topics', label: 'Topics', options: ['a', 'b'], creatable: false } as NamedField;
    await render(FieldInput, { field, frontmatter: { topics: ['a'] }, ...shared() });
  ...
  it('renders an open multiselect as one comma-joined text input with a hint placeholder', async () => {
    const field = { type: 'multiselect', name: 'tags', label: 'Tags', creatable: true } as NamedField;
    await render(FieldInput, { field, frontmatter: { tags: ['x', 'y'] }, ...shared() });
  ...
  it('renders an image field through MediaHeroField, building the hidden src input off the name', async () => {
    const field = { type: 'image', name: 'cover', label: 'Cover' } as NamedField;
    await render(FieldInput, { field, name: 'cover', frontmatter: {}, ...shared() });
    expect(document.querySelector('input[name="cover.src"]')).not.toBeNull();
  });

  it('renders a single reference through ReferenceField (a dialog-popup trigger)', async () => {
  ...
  it('routes an array-of-reference to ReferenceField (an Add popup trigger), not the repeatable list', async () => {
  ...
  it('routes a non-reference array to RepeatableField (its polite live region is present)', async () => {
    const field = { type: 'array', name: 'aliases', label: 'Alias', item: { type: 'text', label: 'Alias' } } as NamedField;
    await render(FieldInput, { field, frontmatter: { aliases: [] }, ...shared() });
    expect(document.querySelector('[role="status"][aria-live="polite"]')).not.toBeNull();
  });

  it('routes an object field to ObjectGroupField (a labeled fieldset legend)', async () => {
    const field = { type: 'object', name: 'meta', label: 'Meta', fields: { note: { type: 'text', label: 'Note' } } } as NamedField;
    await render(FieldInput, { field, name: 'meta', frontmatter: { note: 'hi' }, ...shared() });
    const legend = document.querySelector<HTMLElement>('legend');
    expect(legend?.textContent).toContain('Meta');
    expect(document.querySelector('input[name="meta.note"]')).not.toBeNull();
  });

  it('renders an icon field through IconPicker when the site ships an icon set', async () => {
    const field = { type: 'icon', name: 'marker', label: 'Marker' } as NamedField;
    await render(FieldInput, { field, frontmatter: {}, icons, ...shared() });
    expect(document.querySelector('[role="radiogroup"]')).not.toBeNull();
  });

  it('maps url, email, and datetime to their native input types in the fallback arm', async () => {
    for (const [type, expected] of [['url', 'url'], ['email', 'email'], ['datetime', 'datetime-local']] as const) {
      ...
      const { unmount } = await render(FieldInput, { field, frontmatter: {}, ...shared() });
...
describe('FieldInput validation convention (native required + hint, no inline error chrome)', () => {
  it('surfaces requiredness through the native attribute, with no asterisk or aria-invalid', async () => {
  ...
  it('points aria-describedby at a HINT paragraph built from the field help', async () => {
...
describe('RepeatableField keeps an uncontrolled in-progress edit across a sibling structural change', () => {
  it('preserves an unsaved row edit when a new row is appended (rows are not re-seeded)', async () => {
    await render(FieldInput, { field: faq, name: 'faq', frontmatter: { faq: [{ q: 'first', a: '' }] }, ...shared() });
