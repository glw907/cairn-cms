...
describe('admin css build', () => {
  ...
  it('keeps two of the pinned unlayered rules (menu-focus, guarded-button) by exact selector, out of the full pinned set scripts/checks/custom-surface-budget.json enumerates', () => {
  ...
  it('fixes the desktop sidebar to the viewport, overriding daisyUI\'s own sticky position', () => {
    // The unlayered override (PINNED unlayered rule 3 of the full set scripts/checks/
  ...
  it('fixes the desk-route sidebar to the viewport at xl, its own persist breakpoint', () => {
    // The unlayered `xl:drawer-open` companion (PINNED unlayered rule 4 of the full set
    // scripts/checks/custom-surface-budget.json enumerates, spec §5's desk rider):
