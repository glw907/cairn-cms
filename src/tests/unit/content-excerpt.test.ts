...
import { buildExcerpt, wordCount } from '../../lib/content/excerpt.js';
...
describe('buildExcerpt', () => {
  ...
  it("keeps a container's label, which is where a title slot's prose lives", () => {
    expect(buildExcerpt('::::callout[Avalanche advisory in effect]{tone="warning"}\nCheck the forecast.\n::::')).toBe(
  ...
  it('keeps the whole text of a component whose only slot is its title', () => {
    ...
    expect(buildExcerpt(md)).toBe('Write the post you wish someone had handed you on your first day.');
  ...
  it('degrades sanely when there is no space before maxChars: a hard cut plus an ellipsis', () => {
    ...
    expect(buildExcerpt(body, { maxChars: 10 })).toBe('supercalif…');
  });
