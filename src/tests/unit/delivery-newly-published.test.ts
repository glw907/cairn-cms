// buildNewlyPublished diffs two manifests to find entries a deploy just carried across the
...
import { buildNewlyPublished } from '../../lib/delivery/manifest.js';
...
describe('buildNewlyPublished', () => {
  it('detects an entry newly stamped since before', () => {
    ...
    expect(buildNewlyPublished(before, after)).toEqual([entry({ draft: false, publishedAt: NOW })]);
  });
  ...
  it('does not detect an entry whose stamp was already carried in before', () => {
    ...
    expect(buildNewlyPublished(before, after)).toEqual([]);
  });
  ...
  it('never returns a drafted entry that still carries a stamp from a prior publish, even under the before: null backfill', () => {
    ...
    expect(buildNewlyPublished(null, after)).toEqual([]);
  });
  ...
  it('returns exactly the stamped set when before is null', () => {
    ...
    expect(buildNewlyPublished(null, after)).toEqual([entry({ id: 'stamped', publishedAt: NOW })]);
  });
  ...
  it('matches counterparts by concept+id, so a same-id entry in a different concept is unrelated', () => {
    ...
    expect(buildNewlyPublished(before, after)).toEqual([entry({ concept: 'posts', publishedAt: NOW })]);
  });
});
