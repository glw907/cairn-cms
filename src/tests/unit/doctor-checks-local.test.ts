...
`;
  ...
  // The SvelteKit Cloudflare adapter emits an un-indented `#` comment above its generated path
  // lines; without the comment fix it reads as a bogus zero-header block and the real blanket
  // block below it is never reached.
  const HEADERS_BLANKET_WITH_COMMENT = `# generated by @sveltejs/adapter-cloudflare
/*
  Referrer-Policy: no-referrer
`;
...
`;
  ...
  describe('_headers parser fixes', () => {
    it('fails on a blanket _headers block preceded by an un-indented # comment', async () => {
      const result = await configNoReferrerBlanket.run(
