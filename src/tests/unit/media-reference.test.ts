...
import { parseMediaToken, formatMediaToken } from '../../lib/media/reference.js';
...
describe('parseMediaToken', () => {
  it('round-trips a slug.hash token', () => {
    ...
    expect(formatMediaToken(ref!)).toBe(`media:blue-running-shoes.${HASH}`);
  });
  it('parses and round-trips the bare hash form to a null slug', () => {
    ...
    expect(formatMediaToken(ref!)).toBe(`media:${HASH}`);
  });
...
describe('formatMediaToken', () => {
  it('writes the slug.hash form', () => {
    expect(formatMediaToken({ slug: 'blue-running-shoes', hash: HASH })).toBe(
      `media:blue-running-shoes.${HASH}`,
    );
  });
  it('writes the bare hash form for a null slug', () => {
    expect(formatMediaToken({ slug: null, hash: HASH })).toBe(`media:${HASH}`);
  });
});
