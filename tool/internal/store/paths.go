...
//
// The legacy directory outranks config deliberately: it is where the Node
// CLI (create-cairn-site) still writes records today, so on a platform
// where config diverges from it (darwin, or Linux with XDG_CONFIG_HOME
// set), it is the live registry, and an operator's existing records must
// never go silently unread. An operator who has both directories keeps
// reading the legacy one for as long as it exists; removing it is how an
// operator moves to the config path.
//
// config is os.UserConfigDir in production. Dir returns an error only when
// config itself fails and no legacy directory exists to fall back on.
